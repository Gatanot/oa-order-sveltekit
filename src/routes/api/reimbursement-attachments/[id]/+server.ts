...
import { addAuditLog, getReimbursementAttachment, readAttachmentFile } from '$lib/server/order-db';

export const GET: RequestHandler = (event) => {
  const row = getReimbursementAttachment(event.params.id) as { file_name: string; mime_type: string; storage_path: string } | undefined;
  if (!row) return json({ message: '附件不存在或已被删除' }, { status: 404 });
  addAuditLog({ actorName: '查看模式', action: 'download_attachment', entityType: 'reimbursement', detail: { attachment_id: event.params.id, file_name: row.file_name } });
  let data: Buffer;
