...
function migrate(db: Database.Database) {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS orders(id TEXT PRIMARY KEY, code TEXT UNIQUE NOT NULL, customer TEXT NOT NULL, name TEXT NOT NULL, owner TEXT, stage TEXT NOT NULL CHECK(stage IN ('报价中','执行中','待复验','已验收','待回款','已回款')), contract_amount INTEGER NOT NULL DEFAULT 0 CHECK(contract_amount >= 0), budget_cost INTEGER NOT NULL DEFAULT 0 CHECK(budget_cost >= 0), actual_cost INTEGER NOT NULL DEFAULT 0 CHECK(actual_cost >= 0), created_at TEXT NOT NULL, updated_at TEXT NOT NULL, created_by TEXT NOT NULL DEFAULT 'system', updated_by TEXT NOT NULL DEFAULT 'system', version INTEGER NOT NULL DEFAULT 1 CHECK(version > 0), archived_at TEXT);
...
function riskState(order: OrderDetail): Risk {
  ...
  if (order.workflow.invoice && order.workflow.payment < order.workflow.invoice) reasons.push('回款低于已开票金额');
