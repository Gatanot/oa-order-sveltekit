...
<section>
  <div class="page-head"><div><div class="eyebrow">FINANCE · CLOSE THE LOOP</div><h1>财务跟进</h1><p>项目验收并完成结算后，进入开票与回款跟进。开票不能超过合同额，回款不能超过已开票金额。</p></div><Button variant="primary" disabled={order.stage === '已回款'} onclick={() => workspace.open('finance')}><Wallet size={16} />登记本次开票/回款</Button></div>
  ...
  <section class="panel finance-status"><div class="panel-head"><div><div class="section-kicker">回款闭环</div><h2>开票与回款状态</h2></div><span class="status-pill" class:green={!outstanding} class:orange={Boolean(outstanding)}>{outstanding ? `待回款 ${money(outstanding)}` : '已完成回款'}</span></div>
    <div class="finance-status-grid"><div><small>结算状态</small><b>{order.workflow.settled ? '已结算' : '待结算'}</b><p>{order.stage === '已验收' ? '验收通过后可在工作台完成结算' : '项目完成验收后才能结算'}</p></div><div><small>开票进度</small><b>{money(order.workflow.invoice)} / {money(order.contract_amount)}</b><p>已开票金额不得超过合同额</p></div><div><small>回款进度</small><b>{money(order.workflow.payment)} / {money(order.workflow.invoice)}</b><p>{outstanding ? `合同额还需回款 ${money(outstanding)}` : '合同金额已全部回款'}</p></div></div>
  </section>
