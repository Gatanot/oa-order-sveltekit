<script lang="ts">
  ...
  const outstanding = $derived(Math.max(0, order.contract_amount - order.workflow.payment));
</script>

<section>
  <div class="page-head"><div><div class="eyebrow">LEGACY DATA</div><h1>历史财务记录</h1><p>该模块仅用于查看旧 OA 已导入的开票和回款记录，不参与订单状态、验收或完成判断。</p></div></div>
  ...
  <section class="panel finance-status"><div class="panel-head"><div><div class="section-kicker">回款闭环</div><h2>开票与回款状态</h2></div><span class="status-pill green">仅供查阅</span></div>
    <div class="finance-status-grid"><div><small>结算状态</small><b>{order.workflow.settled ? '已结算' : '待结算'}</b><p>{order.stage === '已验收' ? '验收通过后可在工作台完成结算' : '项目完成验收后才能结算'}</p></div><div><small>开票进度</small><b>{money(order.workflow.invoice)} / {money(order.contract_amount)}</b><p>已开票金额不得超过合同额</p></div><div><small>回款进度</small><b>{money(order.workflow.payment)} / {money(order.workflow.invoice)}</b><p>不作为订单跟踪信息</p></div></div>
  </section>
