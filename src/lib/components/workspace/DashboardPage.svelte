<script lang="ts">
  ...
  const nextAction = $derived(workspace.state.order?.stage === '报价中' ? '创建并确认客户报价' : workspace.state.order?.stage === '执行中' ? (openIssues ? `完成 ${openIssues} 项验收整改` : pendingProcurement ? `完成 ${pendingProcurement} 项采购定标` : '准备并提交复验') : workspace.state.order?.stage === '待复验' ? '等待复验通过或退回整改' : workspace.state.order?.stage === '已验收' ? '完成项目结算' : workspace.state.order?.stage === '待回款' ? '登记回款，完成项目闭环' : '项目已完成');
...
<section class="page-view">
          ...
          <div class="kpi-grid">
            ...
            <div class="kpi"><small>进行中项目</small><strong>{workspace.state.orders.filter((item) => item.stage !== '已回款').length}</strong><span>需要持续推进</span></div>
          ...
          <section class="panel operations-panel">
            <div class="panel-head"><div><div class="section-kicker">订单流程动作</div><h2>订单执行动作</h2></div><span>当前订单：{workspace.state.order!.code}</span></div>
