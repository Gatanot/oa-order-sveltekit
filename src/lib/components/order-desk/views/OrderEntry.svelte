...
<section class="entry-layout">
  <form class="order-form" onsubmit={desk.submitOrder}>
    ...
    <div class="detail-editor"><div class="editor-heading"><h3>员工垫付</h3><button type="button" class="outline-action" onclick={desk.addAdvance}><Plus size={15}/>添加垫付</button></div>
      ...
      {#each desk.advances as advance, i}<div class="repeat-row advance-editor-row"><input value={advance.employee || ''} oninput={(e) => desk.updateAdvance(i, 'employee', e.currentTarget.value)} placeholder="垫付员工" /><input value={advance.item} oninput={(e) => desk.updateAdvance(i, 'item', e.currentTarget.value)} placeholder="垫付物品" /><input type="number" inputmode="decimal" min="0" step="0.01" value={advance.amount} aria-label={`第 ${i + 1} 项垫付金额`} oninput={(e) => desk.updateAdvance(i, 'amount', e.currentTarget.value)} placeholder="金额" /><input class="date-input" type="date" value={advance.date} onclick={(event) => (event.currentTarget as HTMLInputElement).showPicker?.()} oninput={(e) => desk.updateAdvance(i, 'date', e.currentTarget.value)} /><div class="invoice-upload"><label class="upload-action upload-action-small"><Paperclip size={13}/>{advance.invoiceFile ? "重新选择" : "上传发票"}<input type="file" accept="image/png,image/jpeg,image/gif,image/webp,application/pdf,.pdf" disabled={desk.uploadingFiles} onchange={(e) => desk.onAdvanceInvoiceChange(e, i)} /></label>{#if advance.invoiceFile}<a class="upload-name invoice-name" href={URL.createObjectURL(advance.invoiceFile)} target="_blank" rel="noreferrer" title="在新窗口预览"><FileText size={12}/>{advance.invoice}</a><button type="button" class="upload-remove" aria-label={`移除发票 ${advance.invoice}`} onclick={() => desk.removeAdvanceInvoice(i)}><X size={13}/></button>{:else}<small class="field-hint">{advance.invoice || "图片或 PDF"}</small>{/if}</div><span class="field-hint">{advance.status || "待审核"}</span><button type="button" class="delete-action" onclick={() => desk.removeAdvance(i)} aria-label="删除垫付"><Trash2 size={15}/></button></div>{/each}
    </div>
