  ...
  <form class="order-form" onsubmit={desk.submitOrder}>
    ...
    <div class="detail-editor"><div class="editor-heading"><h3>产品明细</h3><button type="button" class="outline-action" onclick={desk.addProduct}><Plus size={15}/>添加产品</button></div>
      ...
      {#each desk.products as product, i}
        <div class="repeat-row editor-row"><div class="autocomplete-field"><input value={product.name} oninput={(e) => desk.updateProduct(i, 'name', e.currentTarget.value)} placeholder="产品名称" required />{#if product.name && productMatches(product.name, i).length}<div class="suggestion-menu">{#each productMatches(product.name, i) as item}<button type="button" onclick={() => desk.applyProductCatalog(i, item.name)}><span>{item.name}</span><small>{item.unit} · ¥{(item.quote_unit / 100).toFixed(2)} · {item.category || "报价库"}</small></button>{/each}</div>{:else if product.catalog_id}<span class="field-hint matched-hint">✓ 已匹配报价库，修改名称将取消匹配</span>{/if}</div><input value={product.unit} oninput={(e) => desk.updateProduct(i, 'unit', e.currentTarget.value)} placeholder="单位" /><input type="number" min="0.01" step="any" value={product.quantity} oninput={(e) => desk.updateProduct(i, 'quantity', e.currentTarget.value)} placeholder="数量" /><input type="number" min="0" step="0.01" value={product.unit_price} oninput={(e) => desk.updateProduct(i, 'unit_price', e.currentTarget.value)} placeholder="销售单价" /><input readonly value={productTotal(product)} placeholder="小计" /><button type="button" class="delete-action" onclick={() => desk.removeProduct(i)} aria-label="删除产品"><Trash2 size={15}/></button><textarea value={product.specification || ''} oninput={(e) => desk.updateProduct(i, 'specification', e.currentTarget.value)} placeholder="规格及制作要求（选填）"></textarea></div>
    ...
    <div class="detail-editor"><div class="editor-heading"><h3>固定厂商成本</h3><button type="button" class="outline-action" onclick={desk.addCost}><Plus size={15}/>添加成本项目</button></div>
      {#if desk.costs.length}<div class="repeat-header editor-row" aria-hidden="true"><span>成本项目 · 可匹配成本库</span><span>供应商</span><span>数量</span><span>成本单价（元）</span><span>小计（元）</span><span></span></div>{/if}
      {#each desk.costs as cost, i}<div class="repeat-row editor-row"><div class="autocomplete-field"><input value={cost.name} oninput={(e) => desk.updateCost(i, 'name', e.currentTarget.value)} placeholder="成本项目" />{#if cost.name && costMatches(cost.name, i).length}<div class="suggestion-menu">{#each costMatches(cost.name, i) as item}<button type="button" onclick={() => desk.applyCostCatalog(i, item.name)}><span>{item.name}</span><small>{item.unit} · ¥{(item.cost_unit / 100).toFixed(2)} · 成本库</small></button>{/each}</div>{:else if cost.catalog_id}<span class="field-hint matched-hint">✓ 已匹配成本库</span>{/if}</div><input value={cost.vendor} oninput={(e) => desk.updateCost(i, 'vendor', e.currentTarget.value)} placeholder="供应商" /><input type="number" min="0.01" step="any" value={cost.quantity} oninput={(e) => desk.updateCost(i, 'quantity', e.currentTarget.value)} placeholder="数量" /><input type="number" min="0" step="0.01" value={cost.unit_price} oninput={(e) => desk.updateCost(i, 'unit_price', e.currentTarget.value)} placeholder="成本单价" /><input readonly value={costTotal(cost)} placeholder="小计" /><button type="button" class="delete-action" onclick={() => desk.removeCost(i)} aria-label="删除成本"><Trash2 size={15}/></button></div>{/each}
    </div>
    <div class="detail-editor"><div class="editor-heading"><h3>员工垫付</h3><button type="button" class="outline-action" onclick={desk.addAdvance}><Plus size={15}/>添加垫付</button></div>
      {#if desk.advances.length}<div class="repeat-header editor-row" aria-hidden="true"><span>垫付物品</span><span>金额（元）</span><span>垫付日期</span><span>发票附件</span><span>状态</span><span></span></div>{/if}
      {#each desk.advances as advance, i}<div class="repeat-row editor-row"><input value={advance.item} oninput={(e) => desk.updateAdvance(i, 'item', e.currentTarget.value)} placeholder="垫付物品" /><input type="number" min="0" step="0.01" value={advance.amount} oninput={(e) => desk.updateAdvance(i, 'amount', e.currentTarget.value)} placeholder="金额" /><input type="date" value={advance.date} oninput={(e) => desk.updateAdvance(i, 'date', e.currentTarget.value)} /><div class="invoice-upload"><label class="upload-action upload-action-small"><Paperclip size={13}/>{advance.invoiceFile ? "重新选择" : "上传发票"}<input type="file" accept="image/png,image/jpeg,image/gif,image/webp,application/pdf,.pdf" disabled={desk.uploadingFiles} onchange={(e) => desk.onAdvanceInvoiceChange(e, i)} /></label>{#if advance.invoiceFile}<a class="upload-name invoice-name" href={URL.createObjectURL(advance.invoiceFile)} target="_blank" rel="noreferrer" title="在新窗口预览"><FileText size={12}/>{advance.invoice}</a><button type="button" class="upload-remove" aria-label={`移除发票 ${advance.invoice}`} onclick={() => desk.removeAdvanceInvoice(i)}><X size={13}/></button>{:else}<small class="field-hint">图片或 PDF</small>{/if}</div><span class="field-hint">待审核</span><button type="button" class="delete-action" onclick={() => desk.removeAdvance(i)} aria-label="删除垫付"><Trash2 size={15}/></button></div>{/each}
    </div>
